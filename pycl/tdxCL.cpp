...
// --- 处理包含关系
void BaoHan(int nCount, float *pDirection, float *pOutHigh, float *pOutLow, float *pInclude, const float *pHigh, const float *pLow) {
...
int LastIsMin(const float *pLow, int fromIndex, int toIndex) {
...
int LastIsMax(const float *pHigh, int fromIndex, int toIndex) {
    for (int i = fromIndex; i < toIndex; i++) {
...
// 反向跳空
int IsReverseJump(int i, int nState, int nLastD, int nLastG, const float *pHigh, const float *pLow) {
    if (nState == 1) {
...
// 从index往前找低点坐标
int GetLastDIndex(int index, const float *pSig) {
...
// 从index往前找高点坐标
int GetLastGIndex(int index, const float *pSig) {
    for (int x = index; x >= 0; x--) {
        if (pSig[x] == 1) {
            return x;
        }
...
// 比较强势的上涨或者下跌也成笔
int IsStrongMove(int i, int nState, int nLastD, int nLastG, const float *pHigh, const float *pLow, float *pSig) {
    if (nState == 1) {
...
bool HasTempBi(int nState, int nCount, int i, float *pHigh, float *pLow, const float *pInclude, const float *pOutHigh, const float *pOutLow) {
    int kCount = 1;
    if (nState == 1)// 找向上笔
    {
...
void Bi2(int nCount, float *pOut, float *pIn, float *pHigh, float *pLow) {
    ...
    auto *pDirection = new float[nCount]();
    auto *pOutHigh = new float[nCount]();
    auto *pOutLow = new float[nCount]();
    auto *pInclude = new float[nCount]();
...
// --- Duan ---
// --- 特征序列画段
void Duan0(int nCount, float *pOut, const float *pIn, const float *pHigh, const float *pLow) {
    int nState = 0;
